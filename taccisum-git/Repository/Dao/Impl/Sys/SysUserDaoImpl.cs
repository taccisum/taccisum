...
{
    [Export(typeof(ISysUserDao))]
    public class SysUserDaoImpl : RepositorySupport<SysUser>, ISysUserDao
    {
        public SysUser LoginVerify(string uid, string psd, EncryptType encryptType)
        {
            var encrypt = EncryptFactory.GetInstance().Create(encryptType);
            var encryptPsd = encrypt.Encrypt(psd);

            var user = Query(u => u.Uid == uid).FirstOrDefault();
            if (user == null || user.Psd != encryptPsd)
                return null;
            return user;
        }
